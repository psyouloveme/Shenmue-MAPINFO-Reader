...
namespace mapinforeader.ColiModels {
    ...
    public class ColiDataType5 : ColiDataSingleCoord {
        ...
        ///<summary>The size (in bytes) of the mystery data in type 5 colis.</summary>
        public static readonly int DATA_SIZE = ColiDataType5.COORD_SIZE + ColiDataType5.MYSTERY_SIZE;
        ...
        public ColiDataType5(uint layerId, BinaryReader reader) : base(layerId, 5) {
            long streamPosition = reader.BaseStream.Position;
            Console.WriteLine("Position is {0}", streamPosition);
            reader.BaseStream.Seek(ColiDataType5.COORD_SIZE, SeekOrigin.Current);

            // read the unkown bytes and seek back to capture it in Data
            this.MysteryBytes = reader.ReadBytes(ColiDataType5.MYSTERY_SIZE);
            reader.BaseStream.Seek(streamPosition, SeekOrigin.Begin);

            // read the full data length (unless we just threw)
            this.Data = reader.ReadBytes(ColiDataType5.DATA_SIZE);
